...
def PSO(J, x0, n_particles, n_iter, w, c1, c2):
   ...
   # Animation function
    def animate(i):
        ...
        plt.xlim(-10, 10)
        plt.ylim(-10, 10)
        plt.quiver(particles[:, 0], particles[:, 1], velocities[:, 0], velocities[:, 1], color='k', scale=10)
        plt.scatter(particles[:, 0], particles[:, 1], c='b')
        plt.scatter(x_best[0], x_best[1], c='r')


        plt.title(f'Iteration {i+1}, w={w_values[i]:.2f} \n Best solution: {x_best[0]:.4f}, {x_best[1]:.4f},Best value: {f_best:.4f}')
...
# Run PSO
x_best, f_best = PSO(J2, x0, n_particles, n_iter, w, c1, c2)
